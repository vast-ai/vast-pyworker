...
def auth_worker(args, worker_metric_map, server_address, api_key, prompt_input, max_new_tokens = 256):
    route_url = f"{server_address}/route/"
    route_payload = {
        "endpoint": args.endpoint_name,
        "api_key": api_key,
    ...
    try:
        ...
        if "url" in message.keys():
            ...
            print(f"no rdy workers ... endpoint status: {message['status']}")
            return
    except json.JSONDecodeError as e:
        print(f"JSON decoding error: {e}")
        print(f"got reponse: {response.text}")
    ...
    if args.backend == "TGI":
        generate_payload["inputs"] = prompt_input
        generate_payload["parameters"] = {"max_new_tokens" : max_new_tokens}
    elif args.backend == "OOBA":
        generate_payload["prompt"] = prompt_input
